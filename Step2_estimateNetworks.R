...
for (c in countries) {
    ...
    adj.mat <- suppressWarnings(cor_auto(df.cnty,
                                         ...
                                         missing = "pairwise",
                                         verbose = FALSE))
    df.mat[[loopfr]] <- abs(adj.mat^2) # soft-thresholding to reduce the effects of non-significant correlations
...
save(df.bts, file = "bootstrapped_graphs.saved") # personal save for later
